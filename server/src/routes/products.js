...
export function productsRoutes({ prisma }) {
  ...
  router.get('/', asyncRoute(async (req, res) => {
    ...
    const [totalItems, products] = await prisma.$transaction([prisma.product.count({ where }), prisma.product.findMany({ where, orderBy: orderBy(query.sort), skip: (query.page - 1) * query.limit, take: query.limit })])
  ...
  router.get('/:handle', asyncRoute(async (req, res) => {
    ...
    const product = await prisma.product.findFirst({ where: { handle, isActive: true } })
