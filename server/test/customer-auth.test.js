...
function createHarness() {
  ...
  const prisma = {
    user: {
      ...
      async create({ data }) {
        user = { id: 'customer-id', designer: null, isDesigner: false, isStaff: false, isSuperuser: false, ...data }
        return { ...user }
      },
...
test('customer registration sends an OTP and activates only after verification', async () => {
  await withServer(async ({ request, sentOtps, getUser }) => {
    ...
    assert.equal(await bcrypt.compare(registration.password, getUser().password), true)
...
test('a pending customer can replace their registration, but an active account cannot', async () => {
  await withServer(async ({ request, sentOtps, getUser }) => {
    ...
    assert.equal(await bcrypt.compare(replacement.password, getUser().password), true)
